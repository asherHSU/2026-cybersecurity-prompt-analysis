...
"""
    ...
    def _unpack_data(self, data) -> List[Dict]:
        """Unpack nested data structures into a flat list of entries."""
        if isinstance(data, list):
            return data
        if not isinstance(data, dict):
            return []

        # MITRE ATT&CK: objects array
        if 'objects' in data and isinstance(data['objects'], list):
            entries = [o for o in data['objects'] if isinstance(o, dict) and o.get('type') in ('attack-pattern', 'course-of-action', 'malware', 'tool', 'intrusion-set')]
            if entries:
                logger.info(f"Unpacked {len(entries)} MITRE objects")
                return entries

        # NVD CVE: vulnerabilities array
        if 'vulnerabilities' in data and isinstance(data['vulnerabilities'], list):
            entries = [v.get('cve', v) for v in data['vulnerabilities'] if isinstance(v, dict)]
            logger.info(f"Unpacked {len(entries)} CVE entries")
            return entries

        # Ubuntu/RSS: entries array
        if 'entries' in data and isinstance(data['entries'], list):
            logger.info(f"Unpacked {len(data['entries'])} RSS entries")
            return data['entries']

        # arXiv: papers array
        if 'papers' in data and isinstance(data['papers'], list):
            logger.info(f"Unpacked {len(data['papers'])} arXiv papers")
            return data['papers']

        # AlienVault OTX: results array
        if 'results' in data and isinstance(data['results'], list):
            logger.info(f"Unpacked {len(data['results'])} OTX results")
            return data['results']

        # CTF: ctftime_events array
        if 'ctftime_events' in data and isinstance(data['ctftime_events'], list):
            logger.info(f"Unpacked {len(data['ctftime_events'])} CTF events")
            return data['ctftime_events']

        # Microsoft: value array
        if 'value' in data and isinstance(data['value'], list):
            logger.info(f"Unpacked {len(data['value'])} Microsoft advisories")
            return data['value']

        # OpenCVE: summary array
        if 'summary' in data and isinstance(data['summary'], list):
            logger.info(f"Unpacked {len(data['summary'])} OpenCVE entries")
            return data['summary']
    ...
    def filter_dataset(self, input_file: Path) -> Tuple[List[Dict], List[Dict]]:
        ...
        process_count = len(data)  # Process all entries
        logger.info(f"Processing all {process_count} entries")
