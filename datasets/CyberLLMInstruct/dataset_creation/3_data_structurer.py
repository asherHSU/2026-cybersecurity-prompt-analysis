...
class CyberDataStructurer:
    ...
    def detect_entry_type(self, entry: Dict) -> Optional[str]:
        ...
        # MITRE ATT&CK: explicit type field
        entry_type_field = entry.get('type', '')
        if entry_type_field in ('attack-pattern', 'malware', 'tool', 'intrusion-set', 'course-of-action'):
            return 'attack_pattern'

        # NVD CVE: id field starts with CVE-
        entry_id = str(entry.get('id', '') or entry.get('cveId', ''))
        if entry_id.upper().startswith('CVE-'):
            return 'vulnerability'

        # Check for CVE pattern anywhere in keys or string values
        if any(key.lower().startswith('cve-') for key in entry.keys()) or \
           any('cve-' in str(value).lower() for value in list(entry.values())[:10]):
            return 'vulnerability'

        # Check for attack pattern indicators
        if any(key in entry for key in {'attack_pattern', 'technique', 'tactic', 'kill_chain_phases'}) or \
           entry_type_field in ('attack_pattern',):
            return 'attack_pattern'

        # Check for security advisory
        if any(key in entry for key in {'advisory', 'bulletin', 'notice', 'usn'}) or \
           any(isinstance(value, str) and 'advisory' in value.lower() for value in list(entry.values())[:10]):
            return 'security_advisory'

        # arXiv / research paper
        if any(key in entry for key in {'summary', 'arxiv_id', 'authors'}) or \
           (isinstance(entry.get('title'), str) and isinstance(entry.get('summary'), str)):
            return 'security_advisory'

        # CTF / generic security challenge
        if any(key in entry for key in {'ctf', 'challenge', 'writeup', 'flag', 'event'}):
            return 'attack_pattern'

        # Fallback: any entry with a description/summary field
        if any(key in entry for key in {'description', 'summary', 'details', 'content'}):
            return 'security_advisory'
